...
def i_joined_the_beatles(yoko):
    i_joined_the_beatles = ['john', 'paul', 'george', 'ringo']
    i_joined_the_beatles.append(yoko)
    return i_joined_the_beatles
...
def we_joined_the_beatles(list):
    we_joined_the_beatles = ['john', 'paul', 'george', 'ringo']
    we_joined_the_beatles.extend(list)
    return we_joined_the_beatles
...
def new_band_member(dict):
    my_dict = {"vocalist": "miss piggy", "lead_guitar": "scooter"}
    my_dict.update(dict)
    return my_dict
...
# Purpose: # Method name: remove_nones_from_dictionary
...
def remove_nones_from_dictionary(dict):
    filtered_dictionary = {key: value for key, value in dict.items() if value is not None}
...
def touch_in(station, time1):
    new_dictionary = {'entrypoint': station, 'time': time1}
